...
// product IS (wall / car / desktop / cable) and then by its power band, because
// that is how the category is merchandised and priced.
//
// Order matters. Cable detection runs first but must NOT catch a charger that
// merely ships with one — "Chargeur + Cable Adeqwat 65W" and "…avec câble" are
...
/**
 * Classify a charger-category product/listing into one of the 7 tiers.
...
export function tierOf(name: string | null | undefined, power: string | null | undefined): TierKey {
  ...
  // 1. Standalone cable — cable word present AND not preceded by a charger word.
  ...
  // 2. Car charger — one tier regardless of wattage.
  if (CAR_WORD.test(n)) return "car";

  // 3. Desktop / station, split at 200W.
  if (DESK_WORD.test(n)) return (w ?? 0) > 200 ? "desk_hi" : "desk_lo";

  // 4. Everything else is a wall charger, split at 100W.
